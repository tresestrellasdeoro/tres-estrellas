...
import {
  ...
  Loader2, RotateCcw, Ticket, CreditCard, Banknote,
  ArrowRight, Printer, Luggage,
} from 'lucide-react'
import { SquareCard, type SquareCardHandle } from '@/components/public/square-card'
import { ALL_STOPS, getPrice, type StopCode } from '@/lib/data/bus-config'
...
interface PassengerRow {
  id: number; full_name: string; passenger_type: PassengerType
  is_promo: boolean; promo_label: string; promo_price: string
}
...
export default function AdminVentaPage() {
  ...
  useEffect(() => {
    fetch('/api/auth/me').then(r => r.json()).then(d => setSucursalId(d.sucursal_id ?? null)).catch(() => {})
    fetch('/api/luggage-types').then(r => r.json()).then(d => { if (d.options?.length > 0) setLuggageOptions(d.options) }).catch(() => {})
  }, [])

  const pricing   = getPrice(origin, destination)
  const mult      = ticketType === 'round_trip' ? 1.5 : 1
  const basePrice = (type: PassengerType) => Math.round((type === 'child' ? pricing.child : pricing.adult) * mult)
  ...
  const addPassenger    = () => { if (passengers.length >= 8) return; const nextId = Math.max(...passengers.map(p => p.id)) + 1; setPassengers(prev => [...prev, { id: nextId, full_name: '', passenger_type: 'adult', is_promo: false, promo_label: 'Promoción', promo_price: '' }]) }
  const removePassenger = (id: number) => passengers.length > 1 && setPassengers(p => p.filter(x => x.id !== id))
  const updatePassenger = (id: number, field: keyof PassengerRow, value: string) => setPassengers(prev => prev.map(p => p.id === id ? { ...p, [field]: value } : p))

  const canSubmit = passengers.every(p =>
    p.full_name.trim().length >= 2 && (!p.is_promo || (p.promo_price !== '' && parseFloat(p.promo_price) > 0))
  ...
  const handleSubmit = async () => {
    ...
    if (paymentMethod === 'card') {
      try { source_id = await squareRef.current?.tokenize() }
      catch (err: any) { setError(err.message ?? 'Error al procesar la tarjeta'); setLoading(false); return }
    }
    ...
    try {
      const res = await fetch('/api/bookings', {
        ...
        body: JSON.stringify({
          ticket_type: ticketType, total_amount: total, luggage_price: luggageTotal,
          luggage_label: luggage.price > 0 ? luggage.label : undefined,
          guest_email: email || undefined, payment_method: paymentMethod, source_id,
          origin_name: ALL_STOPS[origin].name, destination_name: ALL_STOPS[destination].name,
          boarding_stop_code: origin, boarding_stop_name: ALL_STOPS[origin].name,
          sucursal_id: sucursalId ?? undefined, date, departure_time: departureTime,
          return_date: ticketType === 'round_trip' ? returnDate : undefined,
          passengers: (() => {
            const mapped = passengers.map(p => ({
              full_name: p.full_name.trim(), passenger_type: p.passenger_type,
              price: passengerPrice(p), is_promo: p.is_promo,
  ...
  const handleReset = () => {
    setSuccess(null); setEmail(''); setDate(today()); setDepartureTime('8:00 AM')
    setReturnDate(''); setTicketType('one_way')
    setPassengers([{ id: 1, full_name: '', passenger_type: 'adult', is_promo: false, promo_label: 'Promoción', promo_price: '' }])
    setError(''); setPaymentMethod('cash'); setSquareReady(false); setLuggage(NONE_LUGGAGE)
  }
      ...
      <div className="p-4 sm:p-8 max-w-xl mx-auto">
        <div className="bg-white rounded-2xl border border-emerald-200 shadow-sm overflow-hidden">
          <div className="bg-emerald-600 px-6 py-5 text-center">
            ...
            <p className="text-emerald-100 text-sm mt-1">{paymentMethod === 'card' ? 'Pago con tarjeta procesado' : 'Cobrar en efectivo al cliente'}</p>
          </div>
          ...
          {success.qr && (
            <div className="px-6 py-5 text-center border-b border-slate-100">
              <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-3">QR del boleto</p>
          ...
          {email ? (
            ...
            <div className="px-6 py-4 flex items-center gap-2 bg-slate-50 border-b border-slate-100">
              ...
              <p className="text-slate-500 text-sm">Sin correo — mostrar el QR al cliente</p>
            </div>
          )}
...
</head><body><h1>Tres Estrellas de Oro Inc.</h1><hr/><p>Reservación</p><p class="big">${success.booking_number}</p><hr/>
${success.qr ? `<img src="${success.qr}" alt="QR"/>` : ''}
<p>Total: <strong>$${success.total} USD</strong></p><hr/><p style="font-size:10px">Presenta este código al abordar</p></body></html>`)
            ...
            <button onClick={handleReset} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-[#0f2c5c] hover:bg-[#0a1e42] text-white font-bold text-sm transition-colors">
  ...
  return (
    <div className="p-4 sm:p-8 max-w-xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-500 text-sm mt-1">Registro de boleto en caja · Administrador</p>
      </div>
      ...
      <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={origin} onChange={e => setOrigin(e.target.value as StopCode)}
                ...
                {ORIGINS.map(c => <option key={c} value={c}>{ALL_STOPS[c].name}</option>)}
              </select>
            </div>
            <div>
              ...
              <select value={destination} onChange={e => setDestination(e.target.value as StopCode)}
                className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#c01515]/20 focus:border-[#c01515] bg-slate-50">
                {DESTINATIONS.filter(c => c !== origin).map(c => <option key={c} value={c}>{ALL_STOPS[c].name}</option>)}
              </select>
            </div>
          </div>
          {origin !== destination && (
            <div className="mt-3 flex items-center gap-2 bg-slate-50 rounded-xl px-3 py-2 border border-slate-100">
              ...
              <span className="ml-auto text-xs font-black text-[#0a1628]">Adulto: ${basePrice('adult')} · Menor: ${basePrice('child')}</span>
            </div>
          )}
          {origin === destination && <p className="mt-2 text-xs text-red-500 font-semibold">El origen y destino no pueden ser iguales</p>}
        </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {([['one_way', 'Sólo ida'], ['round_trip', 'Ida y vuelta']] as const).map(([val, label]) => (
              <button key={val} onClick={() => setTicketType(val)}
                className={`p-3 rounded-xl border-2 text-sm font-bold transition-all ${ticketType === val ? 'border-[#c01515] bg-[#c01515]/5 text-[#c01515]' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                {label}
              ...
              <div key={p.id} className={`rounded-xl p-4 border-2 transition-colors ${p.is_promo ? 'bg-amber-50 border-amber-300' : 'bg-slate-50 border-transparent'}`}>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    {p.is_promo && <span className="text-[10px] font-black bg-amber-500 text-white px-2 py-0.5 rounded-full">P</span>}
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setPassengers(prev => prev.map(x => x.id === p.id ? { ...x, is_promo: !x.is_promo, promo_price: '' } : x))}
                      className={`text-xs font-black px-2.5 py-1 rounded-lg border-2 transition-all ${p.is_promo ? 'border-amber-400 bg-amber-400 text-white' : 'border-slate-200 text-slate-400 hover:border-amber-300 hover:text-amber-600'}`}>P</button>
                    {passengers.length > 1 && (
                      <button onClick={() => removePassenger(p.id)} className="text-slate-400 hover:text-red-500 transition-colors"><Minus className="w-4 h-4" /></button>
                    )}
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {(['adult', 'senior', 'child'] as PassengerType[]).map(type => (
                      <button key={type} onClick={() => updatePassenger(p.id, 'passenger_type', type)} disabled={p.is_promo}
                        className={`py-1.5 rounded-lg text-xs font-bold transition-all border ${p.is_promo ? 'border-slate-100 text-slate-300 bg-white cursor-not-allowed' : p.passenger_type === type ? 'border-[#c01515] bg-[#c01515]/10 text-[#c01515]' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                        {PASS_LABELS[type]}<span className="block font-black text-[10px]">${basePrice(type)}</span>
                      </button>
                    ))}
                  </div>
                  {p.is_promo && (
                    <div className="mt-2 grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <select value={p.promo_label} onChange={e => setPassengers(prev => prev.map(x => x.id === p.id ? { ...x, promo_label: e.target.value } : x))}
                          className="w-full px-2 py-2 rounded-lg border border-amber-300 bg-white text-xs font-semibold focus:outline-none">
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input type="number" min="0.01" step="0.01" value={p.promo_price}
                            onChange={e => setPassengers(prev => prev.map(x => x.id === p.id ? { ...x, promo_price: e.target.value } : x))}
                            placeholder="0.00"
                            className="w-full pl-6 pr-2 py-2 rounded-lg border border-amber-300 bg-white text-xs font-mono font-bold focus:outline-none" />
                        </div>
                      </div>
                    </div>
                  )}
                  <div className="flex items-center justify-between pt-1">
                    <span className="text-[10px] text-slate-400">{p.is_promo ? `${p.promo_label} · precio especial` : `${PASS_LABELS[p.passenger_type]} · ${ticketType === 'round_trip' ? 'I+V' : 'Ida'}`}</span>
                    <span className={`text-sm font-black ${p.is_promo ? 'text-amber-600' : 'text-[#0a1628]'}`}>${passengerPrice(p) > 0 ? passengerPrice(p).toFixed(2) : '—'}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
          ...
          <p className="text-slate-400 text-xs mb-3">El equipaje de mano siempre es gratuito.</p>
          <div className="space-y-2">
            {luggageOptions.map(opt => {
              const selected = luggage.id === opt.id
              return (
                <button key={opt.id} onClick={() => setLuggage(opt)}
                  className={`w-full text-left flex items-center gap-3 p-3 rounded-xl border-2 transition-all ${selected ? 'border-[#0f2c5c] bg-[#0f2c5c]/5' : 'border-slate-200 hover:border-slate-300'}`}>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setPaymentMethod('cash')}
              className={`flex items-center gap-3 p-4 rounded-xl border-2 text-left transition-all ${paymentMethod === 'cash' ? 'border-[#c01515] bg-[#c01515]/5 text-[#c01515]' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>
              <Banknote className="w-5 h-5 shrink-0" />
              <div><p className="font-bold text-sm">Efectivo</p><p className="text-[10px] opacity-60">Cobrar al cliente</p></div>
            </button>
            <button onClick={() => { setPaymentMethod('card'); setSquareReady(false) }}
              className={`flex items-center gap-3 p-4 rounded-xl border-2 text-left transition-all ${paymentMethod === 'card' ? 'border-[#c01515] bg-[#c01515]/5 text-[#c01515]' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>
              <CreditCard className="w-5 h-5 shrink-0" />
              <div><p className="font-bold text-sm">Tarjeta</p><p className="text-[10px] opacity-60">Square / terminal</p></div>
            </button>
          </div>
          {paymentMethod === 'card' && (
            <div className="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200">
              <p className="text-xs font-bold text-slate-500 mb-2 flex items-center gap-1.5"><CreditCard className="w-3.5 h-3.5" /> Datos de tarjeta</p>
        ...
        <div className="bg-[#0f2c5c] rounded-2xl p-5 text-white flex items-center justify-between">
          <div>
            ...
            <p className="text-white/50 text-xs mt-1">
              {passengers.length} pasajero{passengers.length > 1 ? 's' : ''} · {ticketType === 'round_trip' ? 'Ida y vuelta' : 'Solo ida'} · {ALL_STOPS[origin]?.name} → {ALL_STOPS[destination]?.name} · {paymentMethod === 'card' ? 'Tarjeta' : 'Efectivo'}
            </p>
          </div>
        ...
        <button onClick={handleSubmit} disabled={!canSubmit || loading || (paymentMethod === 'card' && !squareReady)}
          ...
          {loading ? (paymentMethod === 'card' ? 'Procesando pago...' : 'Registrando...') : `Registrar venta · $${total} ${paymentMethod === 'card' ? 'tarjeta' : 'efectivo'}`}
        </button>
